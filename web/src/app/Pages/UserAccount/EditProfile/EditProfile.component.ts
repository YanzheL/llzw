...
export class EditProfileComponent implements OnInit {
  ...
  equal = true;
   ...
   ChangePassword() {
     ...
     if (this.info.valid) {
       this.embryoService.updatePassword({'oldPassword': this.info.value.oldPassword , 'newPassword': this.info.value.newPassword}).subscribe(res => {
         if (res.success) {
           this.embryoService.confirmationPopup('update success').subscribe( res => {
             localStorage.removeItem('rootUser');
             this.router.navigate(['/session/signin']);
           });
         } else {
           this.embryoService.confirmationPopup('update fail, '+ res.error.message).subscribe();
         }
       });
     }
   }
   ...
   submitAddress() {
      if(this.address.valid){
        const data = this.address.value;
        data['owner'] = JSON.parse(localStorage.getItem('rootUser')).username;
        this.embryoService.creatAddress(data).subscribe();
  ...
  onBlur() {
    if (this.info.value.confirmPassword === this.info.value.newPassword){
      this.equal = true;
    } else {
      this.equal  = false;
    }
