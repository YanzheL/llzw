...
public interface StockRepository
  ...
  // SELECT * FROM stocks WHERE productId == ?1 AND inbounded_at IS NOT NULL ORDER BY inbounded_at
  Stream<Stock> findByProductIdAndInboundedAtNotNullOrderByInboundedAt(Product product);

  // SELECT * FROM stocks WHERE productId == ?1 AND inbounded_at IS NOT NULL AND current_quantity >
  // ?2 ORDER BY inbounded_at
  Stream<Stock>
  findByProductIdAndInboundedAtNotNullAndCurrentQuantityGreaterThanEqualOrderByInboundedAt(
      Product product, int quantity);

  // SELECT * FROM stocks WHERE productId == ?1 AND inbounded_at IS NULL ORDER BY inbounded_at
